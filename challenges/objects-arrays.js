...
for (let i=0; i < graduates.length; i++) {
  let fName = graduates[i]['first_name'];
  let email = graduates[i]['email'];
  contactInfo.push(fName + " " + email);
}
...
zooAnimals.forEach(function (zooAnimal) {
  aName = zooAnimal["animal_name"];
  sName = zooAnimal["scientific_name"];
...
zooAnimals.map(function (zooAnimal) {
  ...
  lowerCase.push(aName.toLowerCase());
});
...
zooAnimals.filter(function (zooAnimal) {
  ...
  if (zooAnimal.population < 5) {
    largerPopulation.push(zooAnimal);
  }
});
